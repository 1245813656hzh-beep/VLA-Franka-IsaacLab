...
| 顺序 | 顺序正确 | 堆叠成功 | 颜色正确 |
|---|---|---|---|
| blue → red → green **(训练)** | 10/10 (100%) | 10/10 (100%) | 10/10 (100%) |
...
| **平均** | **36/60 (60%)** | **51/60 (85%)** | **43/60 (72%)** |

### 主要发现

1. **训练模式陷阱**：当 prompt 以 `blue` 开头时（与训练顺序一致），模型会无条件执行训练时的固定顺序 `blue → red → green`，完全忽略 prompt。`blue → green → red` 的顺序正确率为 0%，但堆叠成功率 100%——模型把三个方块都堆好了，只是按训练顺序而非 prompt 顺序。

2. **两个先验而非完整语义**：模型学到的是 **"G 在最后"**（最强先验）和 **"R 在中间"**（次要先验），而非完整的顺序语义。当两个先验同时保持时（如 `red → blue → green`）→ 100%；同时打破时（如 `blue → green → red`）→ 0%。

3. **操控能力与理解能力分离**：堆叠成功率（85%）远高于顺序正确率（60%），说明瓶颈不在操控技能，而在于将语言指令映射到动作序列的能力。模型需要更多样化的顺序训练数据。

> **指标定义**（实现详见 `scripts/inference/eval_gr00t_order_generalization.py`）：
> - **顺序正确**：3 个方块按 prompt 要求的顺序被依次抓取
> - **堆叠成功**：3 个方块全部放入 bin 并堆叠（不考虑顺序）
> - **颜色正确**：未抓取错误颜色的方块（允许任务未全部完成）
