...
数据采集技巧:
    - 键盘控制：I/K(前后) J/L(左右) U/O(上下) 移动，N/M/T/G/Y/B旋转，P 控制夹爪
    - 按 F9 开始/暂停录制，按 E 导出当前 episode，按 R 重置
    - 实时统计会显示动作幅度和接近 0 的比例
    - 目标：有效移动帧 (>1cm) 应 > 50%，接近 0 比例应 < 30%
    - 采集时流畅移动，成功后立即按 E，不要等待
"""
...
# isaaclab_mimic is optional (provides subtask annotations for Mimic data generation)
try:
    import isaaclab_mimic.envs  # noqa: F401
    from isaaclab_mimic.ui.instruction_display import (
        InstructionDisplay as _InstructionDisplay,
        show_subtask_instructions as _show_subtask_instructions,
    )
    _HAS_MIMIC = True
except ImportError:
    _HAS_MIMIC = False
    _InstructionDisplay = None  # type: ignore[misc,assignment]
    _show_subtask_instructions = None  # type: ignore[misc,assignment]
    print("[INFO] isaaclab_mimic not installed. Subtask instruction display will be disabled.")
    print("       To enable: pip install isaaclab-mimic  (or install IsaacLab with mimic extension)")

# Provide aliases that work regardless of whether mimic is present
InstructionDisplay = _InstructionDisplay
show_subtask_instructions = _show_subtask_instructions
    ...
    try:
        if (
            ...
            if args_cli.teleop_device.lower() == "keyboard":
                ...
                cfg = Se3KeyboardCfg(
                    pos_sensitivity=0.02,  # Original sensitivity
...
def run_simulation_loop(
    ...
    def export_and_reset():
        ...
        try:
            ...
            # Analyze the just-exported episode
            if len(action_buffer) > 0:
                actions_array = np.array(action_buffer)
                pos_actions = actions_array[:, :3]
                action_mean_abs = np.abs(pos_actions).mean()
                near_zero_ratio = np.mean(np.abs(pos_actions) < 0.001)

                # Calculate effective motion ratio
                effective_frames = np.sum(np.abs(pos_actions) >= 0.01)
                effective_ratio = effective_frames / len(actions_array)

                print(f"\n{'=' * 60}")
                print(f"Episode 导出完成！帧数：{len(actions_array)}")
                print(f"  平均动作幅度：{action_mean_abs:.4f}m ({action_mean_abs * 100:.2f}cm)")
                print(f"  接近 0 比例：{near_zero_ratio * 100:.1f}%")
                print(
                    f"  有效移动帧 (>1cm): {effective_frames}/{len(actions_array)} ({effective_ratio * 100:.1f}%)"
                )

                if effective_ratio < 0.3:
                    print(f"  ⚠️  警告：有效移动帧少于 30%！建议：")
                    print(f"     - 采集时更快速流畅地移动")
                    print(f"     - 成功后立即按 E，不要等待")
            ...
            print("Episode 手动导出为 SUCCESS 并保存到 HDF5。")
        except Exception as exc:
    ...
    # Statistics tracking for real-time action analysis
    action_buffer = []
    frame_count = 0
    ...
    with contextlib.suppress(KeyboardInterrupt) and torch.inference_mode():
        while simulation_app.is_running():
            ...
            if running_recording_instance:
                ...
                # Print stats every few seconds
                current_time = time.time()
                if current_time - last_print_time >= print_interval and len(action_buffer) >= 30:
                    actions_array = np.array(action_buffer[-30:])  # Last 30 frames
                    action_std = np.std(actions_array[:, :3], axis=0).mean()  # Mean std of position
                    action_max = np.abs(actions_array[:, :3]).max()  # Max position action
                    action_mean_abs = np.abs(actions_array[:, :3]).mean()

                    # Count near-zero actions
                    near_zero_ratio = np.mean(np.abs(actions_array[:, :3]) < 0.001)

                    print(
                        f"\n[实时动作统计] 帧数：{frame_count} | "
                        f"平均幅度：{action_mean_abs:.4f}m | "
                        f"标准差：{action_std:.4f} | "
                        f"最大幅度：{action_max:.4f}m | "
                        f"接近 0 比例：{near_zero_ratio * 100:.1f}%"
                    )

                    if near_zero_ratio > 0.8:
                        print("  ⚠️  警告：80% 以上的动作接近 0！采集时请更大幅度移动机械臂")
                    elif near_zero_ratio < 0.3:
                        print("  ✓ 数据质量良好")

                    last_print_time = current_time

                if subtasks is not None and show_subtask_instructions is not None:
                    if subtasks == {}:
                        subtasks = obv[0].get("subtask_terms")
                    elif subtasks and instruction_display is not None:
