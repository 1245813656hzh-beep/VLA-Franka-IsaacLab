...
def object_lifted(
    ...
    """Check if object is grasped and lifted above a height threshold.

    True when ALL of:
    1. Object is close to the end-effector (grasped/contact state proxy).
    2. Gripper is closed.
    3. Object world Z is above ``min_height``.
    """
    robot: Articulation = env.scene[robot_cfg.name]
    ...
    if hasattr(env.scene, "surface_grippers") and len(env.scene.surface_grippers) > 0:
        surface_gripper = env.scene.surface_grippers["surface_gripper"]
        suction_cup_status = surface_gripper.state.view(-1, 1)
        gripper_closed = (suction_cup_status == 1).squeeze(-1)
    else:
    ...
    close_to_eef = pose_diff < diff_threshold
    above_height = object_pos[:, 2] > min_height
    return torch.logical_and(torch.logical_and(close_to_eef, gripper_closed), above_height)
